...
class ConcreteSUSDCeIntegration(CachedBalancesIntegration):
    def __init__(
        self,
    ...
    def get_block_balances(
        ...
        logging.info("Getting block data for Concrete sUSDCE")
        new_block_data: Dict[int, Dict[ChecksumAddress, float]] = {}
        if not blocks:
            logging.error("No blocks provided to Concrete sUSDCE get_block_balances")
        ...
        for block in sorted_blocks:
            ...
            while start <= block:
                ...
                transfers = fetch_events_logs_with_retry(
                    "Token transfers Concrete sUSDCE",
                    SUSDC_CONTRACT.events.Transfer(),
                    start,
                    to_block,
                )
...
if __name__ == "__main__":
    concrete_susdce_integration = ConcreteSUSDCeIntegration(
        integration_id=IntegrationID.CONCRETE_SUSDC,
        start_block=CONCRETE_START_BLOCK,
        summary_cols=[SummaryColumn.CONCRETE_SUSDC_SHARDS],
        reward_multiplier=20,
    )

    # Without cached data
    without_cached_data_output = concrete_susdce_integration.get_block_balances(
    ...
    with_cached_data_output = concrete_susdce_integration.get_block_balances(
