...
from constants.concrete import (
    CONCRETE_START_BLOCK,
    USDCE_CONTRACT,
    PAGINATION_SIZE,
)
#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
...
class ConcreteUSDCeIntegration(CachedBalancesIntegration):
    def __init__(
        self,
    ...
    def get_block_balances(
        ...
        logging.info("Getting block data for Concrete USDCE")
        new_block_data: Dict[int, Dict[ChecksumAddress, float]] = {}
        if not blocks:
            logging.error("No blocks provided to Concrete USDCE get_block_balances")
        ...
        for block in sorted_blocks:
            ...
            while start <= block:
                ...
                transfers = fetch_events_logs_with_retry(
                    "Token transfers Concrete USDCE",
                    USDCE_CONTRACT.events.Transfer(),
                    start,
                    to_block,
                )
...
if __name__ == "__main__":
    concrete_usdce_integration = ConcreteUSDCeIntegration(
        integration_id=IntegrationID.CONCRETE_USDCE,
        start_block=CONCRETE_START_BLOCK,
        summary_cols=[SummaryColumn.CONCRETE_USDCE_SHARDS],
        reward_multiplier=20,
    )

    # Without cached data
    without_cached_data_output = concrete_usdce_integration.get_block_balances(
    ...
    with_cached_data_output = concrete_usdce_integration.get_block_balances(
